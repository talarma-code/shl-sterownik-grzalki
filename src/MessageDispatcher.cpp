...
bool MessageDispatcher::getRelayStateForEndpoint(uint8_t ep)
{
    switch(ep)
    {
        case 1: return true;        //add implementation 
        case 2: return true;        //add implementation 
    }
    return false;
}
...
void MessageDispatcher::setRelayStateForEndpoint(uint8_t ep, bool state)
{
    switch(ep)
    {
        case 1: state ? heater1.turnOn() : heater1.turnOff(); break;
        case 2: state ? heater2.turnOn() : heater1.turnOff(); break;
        default: Serial.printf("Invalid endpoint for On/Off: %d\n", ep); break;
    }
}
